...
```{r}
##########################################
# Parsing files published 2001 and later
##########################################
require(hunspell)
require(pdftools)
require(stringr)
...
write.table(txt_frame, "Ksummary_text.csv", sep = ",", col.names = T, row.names = F)
...
if (count > 0){
       txt_frame <- data.frame(KNUMBER = K_number, txt = txt, 
                                Kcount = count, stringsAsFactors = F)
# if file is not accessible don't record the data       
} else if ((txt == "error in openconnectioncon rb cannot open connection") & count == 0){
       txt_frame = NULL
       
# if the file seem to be accesible but could't read the pdf, the file could be image
# try to recover data using text recognition       
} else {
