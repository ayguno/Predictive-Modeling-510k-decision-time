...
Since we don't know which available PDF files will be readable we will first attempt reading directly from the source, if this fails, then we will start downloading the pdfs as highresolution pngs, perform text recognition and read them afterwards. 

We should attempt to remove all special characters and retain only alphanumeric characters in the text before storing them in order to reduce the storage burden.
